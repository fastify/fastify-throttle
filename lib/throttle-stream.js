...
function ThrottleStream (opts) {
   ...
   * @default null
   */
  this.bytesPerSecondFn = typeof opts?.bytesPerSecond === 'undefined'
...
ThrottleStream.prototype._init = function _init () {
  ...
  this._interval = setInterval(() => {
    this._allowedBytes = this.bytesPerSecondFn((Date.now() - this.startTime) / 1000, this.bytes)

    if (
      this._allowedBytes !== 0 &&
      this._buffer !== null
    ) {
      setImmediate(() => this._transform())
    }
  }, 1000)
}
