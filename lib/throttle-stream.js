...
ThrottleStream.prototype._transform = function (chunk, encoding, callback) {
  if (this.startTime === null) {
    ...
    this._interval = setInterval(() => {
      this._allowedBytes = this.bpsFn(Math.floor((Date.now() - this.startTime) / 1000), this.bytes)
      if (
        this._allowedBytes !== 0 &&
        this._buffer !== null
      ) {
...
ThrottleStream.prototype._process = function () {
  ...
  const toBeSend = chunkLength - (chunkLength - this._allowedBytes)
  if (toBeSend === 0) {
    return
  }
  this.push(this._buffer.slice(0, toBeSend))
  this._buffer = this._buffer.slice(toBeSend)
